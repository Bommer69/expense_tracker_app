...
function createTools(userId) {
  return [
    // Tool 1: Tổng quan thu chi tháng
    tool(
      async ({ month }) => {
      ...
      },
      {
        ...
        schema: z.object({
          month: z.string().optional().describe('Tháng cần xem, format YYYY-MM (vd: 2025-05). Không truyền = tháng hiện tại.'),
        }),
      }
    ),

    // Tool 2: Chi tiết theo danh mục
    tool(
      async ({ month, type }) => {
        const m = month || currentMonth();
        const { start, end } = monthRange(m);
      ...
      },
      {
        ...
        description: 'Chi tiết thu/chi theo từng danh mục trong tháng. Dùng khi hỏi chi tiêu nhiều nhất ở đâu, danh mục nào tốn nhiều tiền nhất.',
        schema: z.object({
          month: z.string().optional().describe('Tháng cần xem, format YYYY-MM. Mặc định tháng hiện tại.'),
          type: z.enum(['income', 'expense']).optional().describe('income = thu nhập, expense = chi tiêu. Mặc định: expense.'),
        }),
      }
    ),

    // Tool 3: Trạng thái ngân sách
    tool(
      async ({ month }) => {
        const m = month || currentMonth();
        ...
        return `Ngân sách tháng ${m}:\n${lines.join('\n')}\n${over > 0 ? `⚠️ Vượt ngân sách: ${over} danh mục` : '✅ Chưa vượt ngân sách'}`;
      },
      {
    ...
    // Tool 4: Giao dịch gần đây
    tool(
      async ({ limit, type, category_name }) => {
        const query = { userId };
      ...
      },
      {
        ...
        description: 'Lấy danh sách giao dịch gần đây, có thể lọc theo loại hoặc danh mục. Dùng khi hỏi về các giao dịch cụ thể.',
        schema: z.object({
          limit: z.number().optional().describe('Số lượng giao dịch (tối đa 20). Mặc định: 10.'),
          type: z.enum(['income', 'expense']).optional().describe('Lọc: income hoặc expense.'),
          category_name: z.string().optional().describe('Lọc theo tên danh mục (vd: Ăn uống, Đi lại).'),
        }),
      }
    ),

    // Tool 5: Xu hướng nhiều tháng
    tool(
      async ({ months }) => {
      ...
      },
      {
        ...
        description: 'Xem xu hướng thu chi qua nhiều tháng liên tiếp. Dùng khi so sánh chi tiêu các tháng hoặc hỏi về xu hướng tài chính.',
        schema: z.object({
          months: z.number().optional().describe('Số tháng cần xem (tối đa 6). Mặc định: 3.'),
        }),
      }
    ),
  ];
