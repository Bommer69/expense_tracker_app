...
const { generateDailySummary, evaluateAnomalies, generateMonthlySummary } = require('./services/aiTriggerService');
  ...
  .then(async () => {
    ...
    setInterval(async () => {
      try {
        ...
        // Monthly rollover + summary: chạy lúc 00:00-01:00 ngày đầu tháng
        if (hour === 0 && day === 1) {
          console.log('[Scheduler] Running monthly rollover...');
          await monthlyRolloverCheck();
          console.log('[Scheduler] Monthly rollover completed');

          // Tổng kết tháng trước bằng AI
          console.log('[Scheduler] Running monthly AI summaries...');
          const users = await User.find({}).select('_id');
          for (const user of users) {
            try {
              await generateMonthlySummary(user._id);
            } catch (err) {
              console.error(`[Scheduler] monthly summary failed for user ${user._id}:`, err.message);
            }
          }
          console.log(`[Scheduler] Monthly AI summaries done for ${users.length} users`);
        }
